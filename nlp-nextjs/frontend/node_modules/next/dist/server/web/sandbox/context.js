        ...
        extend: (context)=>{
            ...
            context.__next_eval__ = function __next_eval__(fn) {
                ...
                if (!warnedEvals.has(key)) {
                    const warning = (0, _middleware).getServerError(new Error(`Dynamic Code Evaluation (e. g. 'eval', 'new Function') not allowed in Middleware`), "edge-server");
            ...
            context.__import_unsupported = function __import_unsupported(moduleName) {
...
function emitWarning(name, contextOptions) {
    if (!warnedAlready.has(name)) {
        const warning = new Error(`A Node.js API is used (${name}) which is not supported in the Edge Runtime.
